...
object BamFileSetup {
  ...
  val bamFile = filePath"/path/to/test".bam
...
object SharpTest {
 ...
 val bamFile = filePath"${path}/DownSmp_Flex300".bam
 val genomeFile = filePath"${path}/human".genome
 val ref = filePath"/stash/usit/cluster/projects/p22/dev/p22-tomegil/ref/hum37/human_g1k_v37_decoy".fasta
...
object BioCookbookSpec {
  ...
  val suite1 = suite(
    ...
    test("map and align") {
      ...
      val output = filePath"/path/to/sample".markdup.indexed.sorted.bam
      val metricsFile = filePath"/path/to/metrics.txt"
